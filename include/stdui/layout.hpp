...
auto measure_stack(Range const &children, proposal const &proposal) -> layout_result {
...
template <std::ranges::input_range Range>
  requires layout_element<std::ranges::range_value_t<Range>>
auto measure_hstack(Range const &children, proposal const &proposal) -> layout_result {
  return detail::measure_stack<detail::stack_axis::horizontal>(children, proposal);
}

/// Measures vertical children and distributes height by flexibility.
template <std::ranges::input_range Range>
  requires layout_element<std::ranges::range_value_t<Range>>
auto measure_vstack(Range const &children, proposal const &proposal) -> layout_result {
  return detail::measure_stack<detail::stack_axis::vertical>(children, proposal);
}
...
auto layout_hstack(Range const &children, rect const &bounds,
                   layout_direction direction = layout_direction::left_to_right,
                   layout_alignment alignment = layout_alignment::start) -> arranged_layout {
  auto measurement =
      measure_hstack(children, proposal::bounded(bounds.extent.width, bounds.extent.height));
  auto frames = arrange_hstack(measurement.children, bounds, direction, alignment);
...
auto layout_vstack(Range const &children, rect const &bounds,
  ...
  auto frames = arrange_vstack(measurement.children, bounds, alignment);
  return {std::move(measurement), std::move(frames)};
}
